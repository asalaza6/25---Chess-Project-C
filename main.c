...
void playMove(PIECES* player, BOARD* gameBoard){
	...
	}while(playerMove->piece->color != player->color);	 
	//move said piece
    do
    {
...
int main(){
  ...
  while(1){
	...
	/*do{
		...
		printf("Type = %c color = %d myColor = %d\n",playerMove->piece->type, playerMove->piece->color,playerColor);
	}while(playerMove->piece->color != playerColor);	 
	//move said piece
    do
    {
    	printf("Where do you want to move it?\n");
	...
	movePiece(playerMove, gameBoard);*/
	//my makeshift move function
	// gameBoard->array[playerMove->dest]=playerMove->piece;
	// gameBoard->array[playerMove->from] = NULL;
